...
export function initMatrix(brightness = 85) {
  ...
  const matrixOptions = {
    ...
    pwmLsbNanoseconds: 300,  // Higher value = more stable brightness
    pwmDitherBits: 0,
    limitRefreshRateHz: 120, // Cap refresh rate for stability
  };
...
export function clear() {
  if (matrix) {
    matrix.clear();
    // Reapply brightness after clear to prevent desync
    matrix.brightness(currentBrightness);
  }
}
